<template>
  <div class="friendSearchContainer">
    ...
    <div v-if="invalidData"
         class="invalid-msg">{{invalidData}}</div>
    <ul v-show="searching"
...
export default {
  ...
  methods: {
    ...
    scrollViewport() {
      ...
      this.$el.querySelector(".friendSearchList").scrollTop =
        (this.focusIndex - 1) * 26;
    },
    // 鼠标hover到下拉list
    hoverOn(e) {
      const ul = this.$el.querySelector(".friendSearchList");
    ...
    selectOne(e) {
      clearTimeout(this.timer);
      const target = this.$el.querySelectorAll(".friendSearchList li")[
        this.focusIndex
      ];
      this.chooseOne(target);
    }
...
/* smartInput输入框需要的样式表 */
.friendSearchContainer input, .friendSearchContainer ul, .friendSearchContainer li, .friendSearchContainer div {
...
import SmartInput from "./SmartInput/SmartInput.vue";
const provinceList = [
  "北京市",
  "天津市",
  "上海市",
